...
OneDock 是专为**中小型团队**和**单机容器部署**场景设计的轻量级容器编排解决方案，完美填补了 Docker 和 K8s 之间的空白：

#### 🔥 核心应用场景

**1. 小规模容器编排**
- 单机或少量服务器的容器部署需求
- 不需要 K8s 复杂性，但需要比 Docker Compose 更强的 API 控制能力
- 希望通过 RESTful API 而非配置文件管理容器

**2. CI/CD 自动化部署**
- 通过 API 实现与现有 CI/CD 流水线的无缝集成
- 支持蓝绿部署、滚动更新等企业级部署策略
- 实时监控部署状态，快速回滚异常服务
...
**简单但不简陋：**
- 单一二进制文件，5 分钟完成安装
- RESTful API 设计，与任何技术栈集成
- 智能负载均衡，自动适配单副本/多副本场景
- 内置健康检查和监控，运维负担最小

**强大且灵活：**
- 支持滚动更新，保证服务连续性
- 动态扩缩容，无需重启服务
- 完整的 Swagger 文档，支持在线测试
- 多种负载均衡策略，满足不同性能需求
